...
for i in \
  ...
  www/php${PHP_VER}-tidy \
    ; do
  echo "+ $i"
  if [[ ! -d $DIR_CACHE/$i ]]; then
    ghget $DIR_CACHE/$i $URL_GIT/$i || exit $?
  fi
  if [[ -d $DIR_PORTS/$i ]]; then
    rm -rf $DIR_PORTS/$i
  fi
  cp -Rp $DIR_CACHE/$i $DIR_PORTS/$i || exit $?
done
echo '= php.mk'
phpmk=$DIR_CACHE/php.mk.$PHP_VER
if [[ ! -f $phpmk ]]; then
  ghget $phpmk ${URL_GIT/tree/blob}/Mk/Uses/php.mk || exit $?
fi
cp $phpmk $DIR_PORTS/Mk/Uses/php.mk
